...
interpret_colverify() {
  ...
  local peak avg worst10_avg best90_avg avg_L avg_a avg_b
  peak=$(echo "$output" | awk '/^ *Total errors:/{for(i=1;i<=NF;i++) if($i=="peak"){v=$(i+2);sub(/,/,"",v);print v;exit}}')
  ...
  if [ -n "$avg_L" ] && [ -n "$avg_a" ] && [ -n "$avg_b" ] && ! float_lt "$avg" 2.0; then
    ...
    if float_lt "$dom_val" "$avg_a"; then
      dominant="a*"; dom_val="$avg_a"; dom_desc="green-red (chromatic)"
    fi
    if float_lt "$dom_val" "$avg_b"; then
      dominant="b*"; dom_val="$avg_b"; dom_desc="blue-yellow (chromatic)"
    fi
    echo
    echo "  The dominant error is in $dom_desc ($dominant = $dom_val)."
    case "$dominant" in
      "L*") echo "  This suggests a tone response curve (TRC) difference between"
            echo "  the Wayland and X11 gamma ramp handling." ;;
      "a*") echo "  This suggests a difference in how red/green channel calibration"
            echo "  curves are being applied." ;;
      "b*") echo "  This suggests a difference in how blue channel calibration"
            echo "  curves are being applied." ;;
    esac
  fi
...
pick_display_type() {
  echo
  echo "$(bold "Select your display backlight type:")"
...
pick_instrument() {
  echo
  echo "$(bold "Detecting instruments...")"
...
do_measurement() {
  ...
  echo
  echo "$(bold "--- Measuring: $description ---")"
  echo
...
step_measure_wayland_cal() {
  echo
  echo "$(bold "=== Measure Wayland Calibrated ===")"
  echo
...
step_measure_x11_cal() {
  echo
  echo "$(bold "=== Measure X11 Calibrated ===")"
  echo
  ...
  dispwin "$ICC_PROFILE"
  echo "$(green "VCGT loaded via dispwin.")"
...
step_compare() {
  ...
  if [ "$i" -lt 2 ]; then
    echo
    echo "$(yellow "Need at least 2 measurement files to compare.")"
    echo "Run more measurement steps first."
    return
  fi

  echo
  echo "$(bold "Select comparisons to run:")"
  echo
  ...
  if [ -f "$wl_cal" ] && [ -f "$x11_cal" ]; then
    echo "$(bold "--- Wayland vs. X11 (both calibrated) ---")"
  ...
  if [ -f "$wl_cal" ] && [ -f "$ICC_PROFILE" ]; then
    echo "$(bold "--- Wayland calibrated vs. ICC profile ---")"
  ...
  if [ -f "$x11_cal" ] && [ -f "$ICC_PROFILE" ]; then
    echo "$(bold "--- X11 calibrated vs. ICC profile ---")"
    echo
  ...
  if [ "$ran_comparison" -eq 0 ]; then
    echo "$(yellow "No recognized file pairs found for automatic comparison.")"
...
pick_session() {
  echo "$(bold "Existing sessions in $VALIDATION_DIR:")"
  echo
  ...
  for d in "$VALIDATION_DIR"/*/; do
    ...
    sessions+=("$d")
    local files
    files="$(ls "$d"*.ti3 2>/dev/null | wc -l)"
...
main_menu() {
  while true; do
    echo
    echo "$(bold "=========================================")"
    echo "$(bold " VCGT Calibration Validation")"
    echo "$(bold "=========================================")"
    echo
...
cmd_create() {
  echo
  echo "$(bold "VCGT Calibration — Create Measurements")"
  echo "$(bold "=======================================")"
  echo

  echo "$(bold "Checking tools...")"
  check_tools
  echo
...
cmd_compare() {
  echo
  echo "$(bold "VCGT Calibration — Compare Results")"
  echo "$(bold "===================================")"
  echo
  ...
  if [ -n "$ICC_PROFILE" ]; then
    ...
    echo "$(yellow "No ICC profile set — profcheck comparisons will be skipped.")"
  fi
...
cmd_interactive() {
  echo
  echo "$(bold "VCGT Calibration Validation Script")"
  echo "$(bold "===================================")"
  echo
  ...
  echo
  echo "$(green "Configuration saved.")"
